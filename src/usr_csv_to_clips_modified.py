...
group = hinUsrCsv[0].strip().split(',')          # row 1: grouping/chunking
conceptDict = hinUsrCsv[1].strip().split(',')    # row 2: concept dict info
wid = hinUsrCsv[2].strip().split(',')            # row 3: group/chunk ids
pos = hinUsrCsv[3].strip().split(',')            # row 4: def/pos? info
gnp = hinUsrCsv[4].strip().split(',')            # row 5: GNP info
intraChunkRels = hinUsrCsv[5].strip().split(',') # row 6: intra chunk relations
interChunkRels = hinUsrCsv[6].strip().split(',') # row 7: inter chunk relations
discorseRel = hinUsrCsv[7].strip().split(',')    # row 8: discourse relations
...
ans.write('(sentence_type\t' + hinUsrCsv[8].strip() + ')\n')
