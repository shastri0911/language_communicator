     ...
     with open(fname,'r') as f:
        ...
        for i in a:
                if i not in me:
                        ...
                        n = str(me).strip('[]')
                        n = n.replace("\'","")
        for index, item in enumerate(me, start = 1):#To store the location of each word.For ex:"This is an apple."This-1,is-2,an-3,apple-4"
                n = str(item).strip('[]')
                n=n.replace("\'","")
                st = re.findall(r'\:.*?\>',n)
		st1 =re.findall(r'\<.*?\:',n)
...
def _loc(line):  #For lines in MRS that contains all nodes. (For ex: "This is an apple." => [ _apple_n_1<11:17> LBL: h12 ARG0: x8 ] )
#	p1 = str(sys.argv[2])
	p= open( p1,'a+')
	f1 = []
...
def _arg(line):  #To store details of all the ARGs
 #       p1 = str(sys.argv[2])
        p= open( p1,'a+')
	n= '(mrs_id-Args '                               ###[ _xx_x<?:?> LBL:? ARG0:? [e SF:? TENSE:? PERF: ? ] ARG1:?]
        p.write(n)
        p.write(" ")
        if (line.find("ARG0: ")!=-1):
                  k=line.index("ARG0: ")
